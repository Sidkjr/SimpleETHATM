...
export default function HomePage() {
  ...
  const [payloanAmount, setPayLoanAmount] =  useState(undefined);
  ...
  const getCredit = async() => {
    if (atm) {
      setCredit((await atm.getCredit()).toNumber());
    }
  }
  ...
  const getLoan = async() => {
    if(atm) {
      let tx = await atm.getloan(loanAmount);
      await tx.wait();
      getBalance();
      getCredit();
    }
  ...
  const payLoan = async() => {
    if(atm) {
      let tx = await atm.payLoan(payloanAmount);
      await tx.wait();
      getBalance();
      getCredit();
    }
  ...
  return (
    <main className="container">
      <header><h1>Welcome to the Bank ATM!</h1></header>
      {initUser()}
      <style jsx>{`
        .container {
          text-align: center
